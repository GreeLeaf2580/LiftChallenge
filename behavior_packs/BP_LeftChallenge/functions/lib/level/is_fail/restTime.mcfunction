...
execute unless block ~~2~ quartz_slab [] if score @s restTime matches 1.. run scoreboard players remove @s restTime 1
...
execute if block ~~2~ quartz_slab [] run scoreboard players operation @s restTime > failTime active
...
#如果玩家在关卡里，且倒计时归零
    #如果玩家脚下是空气，则提示玩家飘走了
    execute if block ~~-1~ air [] if score @s restTime matches 0 run tellraw @s { "rawtext": [{ "translate": "tell.failmsg.lift_out", "with": { "rawtext": [{ "selector": "@s" }] } }] }
