...
execute as @a[scores={section=!0}] at @s if entity @s[y=10,dy=-80] run tellraw @s { "rawtext": [{ "translate": "tell.failmsg.out_world", "with": { "rawtext": [{ "selector": "@s" }] } }] }

execute as @a[scores={section=!0}] at @s if entity @s[y=10,dy=-80] run function items/reset
